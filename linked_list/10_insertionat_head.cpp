...
//this means add val before the head ie if 5 element then 6 should be there.
Node* insertHead(Node* head,int val){

    Node* temp=new Node(val,head);// so this will be the new node at first with val initialized and the member next initialized to head of supplied node.
    //so this new node have value and next initialized to resp. value with next pointing to previously first element of node.

    return temp; //so this is our new head we returned.


    //insted can do directly 'return new Node(val,head);' instead of above 2 lines.
}
...
int main(){
  ...
  head=insertHead(head,17); //can do directly 'head=new Node(17,head);' this is automatically creating a new node and head is pointing to this.
  print(head);
